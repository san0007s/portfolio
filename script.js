...
document.addEventListener('DOMContentLoaded', function () {
	const roles = ['Front End Developer', 'Data scientist', 'Software Engineer', 'Machine Engineer', 'AI enthusiast'];
	let currentIndex = 0;
	...
	function typeRole(role, callback) {
	  ...
	  function type() {
		if (charIndex < role.length) {
		  ...
		  setTimeout(callback, 1000); // Wait before starting to erase
		}
	  }
	...
	function eraseRole(callback) {
	  ...
	  function erase() {
		if (roleElement.textContent.length > 0) {
		  ...
		  setTimeout(callback, 500); // Wait before starting to type next role
		}
	  }
